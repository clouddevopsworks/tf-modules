...
def createRegistry(tfWorkspace) {
    dir("${env.repo_path}"){
...
def destroyRegistry(tfWorkspace) {
    dir("${env.repo_path}"){
...
pipeline {
    ...
    stages {
            ...
            parallel {
                stage('Provisioning') {
                    ...
                    steps {
                        ...
                        {
                            setEnv("eu-central-1", "user-app", "registry", "poc")
                            sh "env"
                ...
                stage('De-provisioning') {
                    ...
                    steps {
                        ...
                        )
                        {
                            setEnv("eu-central-1", "user-app", "registry", "poc")
                            sh "env"
                            destroyRegistry("default")
                        } 
                    }
                }
            }
        }
        stage('Registry - Acceptance') {
            parallel {
                stage('Provisioning') {
                    when {
                        expression { 
                    ...
                    steps {
                        ...
                        {
                            setEnv("eu-central-1", "user-app", "registry", "acc")
                            sh "env"
                            createRegistry("default")
                            output()
                        }
        ...
        stage('Registry - Production-Staging') {
            parallel {
                stage('Provisioning') {
                    when {
                        expression { 
                    ...
                    steps {
                        ...
                        {
                            setEnv("eu-central-1", "user-app", "registry", "prd-stg")
                            sh "env"
                            createRegistry("default")
                            output()
                        }
            ...
            parallel {
                stage('Provisioning') {
                    ...
                    steps {
                        ...
                        {
                            setEnv("eu-central-1", "user-app", "registry", "prd")
                            sh "env"
                            createRegistry("default")
                            output()
                        }
